...
async function main()
{
	...
	let fileName = "../03-计算所有翻页链接/翻页地址数组.txt";
	...
	for(let index = 0 , len = item_list.length ; index < len ; ++index )
	{
		...
		console.log("爱马仕陈列 翻页 进度" , index + 1 , "/", len);
		
		let want_out_file = `./pages/${one_item.fileName}.txt`;
		if(fs.existsSync(want_out_file))
			continue;
		
		//测试
		console.log("当前" , one_item.fileName);
		//one_item.words = "PRECOLLECTIONPE";
		
		let want_link = one_item.link;
